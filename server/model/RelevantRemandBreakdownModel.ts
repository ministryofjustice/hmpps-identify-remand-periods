...
import RemandBreakdownModel, { createRemandBreakdown } from '../views/components/remand-breakdown/RemandBreakdownModel'

export default class RelevantRemandBreakdownModel extends RemandCardModel {
  ...
  public relevantRemandBreakdowns: RemandBreakdownModel[]

  public notRelevantRemandBreakdowns: RemandBreakdownModel[]
  ...
  constructor(
    ...
    this.adjustments = RelevantRemandBreakdownModel.getAdjustments(relevantRemand)
    this.remandTable = createRemandTableFromAdjustments(this.adjustments, relevantRemand.charges, true)
    this.relevantRemandBreakdowns = createRemandBreakdown(this.relevantChargeRemand)
    this.notRelevantRemandBreakdowns = createRemandBreakdown(this.notRelevantChargeRemand)
        ...
        return [
          {
            text:
              ...
              dayjs(it.from).format('DD/MM/YYYY'),
          },
          {
            text: dayjs(it.to).format('DD/MM/YYYY'),
          },
          {
  ...
  public periodsOutOfPrisonTable() {
    return {
      ...
      rows: this.periodsOutOfPrison.map(it => {
        return [
          {
            text: dayjs(it.from).format('DD/MM/YYYY'),
          },
          {
            text: dayjs(it.to).format('DD/MM/YYYY'),
          },
